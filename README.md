...
**Complete Swap** [completeSwap(address, amount)](https://github.com/cooperativ-labs/share-manager-contract-algorand/blob/main/index.rsh#:~:text=addWL%3A%20Fun(%5BAddress,Bytes(96)%5D%2C%20Bool)%2C) - This is a restricted function for buying share tokens from an open sale or from an initiated swap. Only contract creator/manager or whitelisted investors can use this API. This function takes in two arguments (address of the initiator of the swap, and amount of shares to be purchased). The function returns a boolean `true` if the call succeeds..
...
**View Current Swap Details** [vCurrSwap(address)](https://github.com/cooperativ-labs/share-manager-contract-algorand/blob/main/index.rsh#:~:text=const%20Views%20%3D%20View,%7D) - This function takes in one argument (address of swap initiator), and returns array [`some/none`, `{ qty: BigNumber, price: BigNumber, sold: BigNumber, status: 'initd', cumProceeds: BigNumber, shareIssuanceType: Boolean }`]. `cumProceeds` represents backing token balance from all swaps or share token purchases. `sold` represents the amount of shares already sold so far in the current swap/sale. `status` represents the current status of each swap and it can have the following possible values: `-----` which implies there is no current swap, `initd` which implies swap has been initiated by seller, `apprv` which implies that swap has been approved by the syndicator, `partl` which implies that some buyer(s) has/have bought some of the shares offered for sale/swap but there are still some shares remaining, and `compl` which implies that buyer has bought swap and swap has been completed.
